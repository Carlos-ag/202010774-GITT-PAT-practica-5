...
public class PortfolioManager {
    ...
    public PortfolioManager(ArrayList<PorfolioMovement> movements) {
        this.movements = movements;
    }

    public PortfolioManager()  {
        ArrayList<PorfolioMovement> movements = getPortfolioMovementsFromCSV();
        this.movements = movements;
    }


private ArrayList<PorfolioMovement> getPortfolioMovementsFromCSV() {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
        while ((line = br.readLine()) != null) {
            ...
            UUID id = Converter.convertStringToUuid(movementData[0].replaceAll("\"", ""));
            Stock stock = new Stock(movementData[1].replaceAll("\"", ""));
            int quantity = Integer.parseInt(movementData[2].replaceAll("\"", ""));
            double price = Double.parseDouble(movementData[3].replaceAll("\"", ""));
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            Date date = sdf.parse(movementData[4].replaceAll("\"", ""));
            PorfolioMovement movement = new PorfolioMovement(id, stock, quantity, price, date);
...
public void appendPortfolioMovementToCsv(PorfolioMovement movement) {
    String csvFile = CSV_FILE;
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
